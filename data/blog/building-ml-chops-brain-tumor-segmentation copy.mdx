...
## Show off widget that executes my cloud model
<BrainTumorSegmentationV1Widget />
  ```languages: text
  ### Widget spec
  - Single button "Go"
    - Picks a random image file from the validation set
    - Makes a request to replicate API
    - Displays the image w/ label overlaid on the left, w/ ground truth on the right flashing to indicate progress
    - Displays a progress bar hardcoded to progress at a rate of 1 compl. per 65s
    - Render text "Replicate cog instance loading..."
    - Poll replicate every 5s, when response is found, download and render a mask applied image instead of the ground truth.
  ```
## Reflection: 
  ### Process changes
  ...
  ##### Write the blog post _as_ I do the work
    I'm not sure if I'd want to read this blog post. I'm not sure that I'd really want to read my 'Learning Principles' nor my focuses for the week. It's necessary for journaling though 🤷‍♀️. I guess the blog post will need be iterated on as well.
  ### Things to remember...
